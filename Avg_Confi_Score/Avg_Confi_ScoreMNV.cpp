...
int main(int argc, char** argv)
{
    ...
    {
        pcTmp=strchr(lines,'\r');  if (pcTmp)*pcTmp=0;
        pcTmp=strchr(lines,'\n');  if (pcTmp)*pcTmp=0; 

        if (lines[0]=='#'||lines[0]==0||lines[0]=='C') continue;

        chomp(lines);
        cnt = rowParser(lines, "\t", fields, 100);
        
           
        //if (fields[0][0] == '#'||fields[0][0] == '\r'|| fields[0][0] == '\n'||fields[0][0] == 'C')
        //    continue;
        
        sr = (int)atoi(fields[7]);
