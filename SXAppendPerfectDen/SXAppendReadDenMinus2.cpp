...
static void banner(char *argv[])
{
    ...
    printf("Built %s %s\n", __DATE__, __TIME__);
    printf("Usage: %s\n", argv[0]);
    printf("\t<Input filename> <Read Density files path> <Type :- S,I,D> <Sample:- 1 or 2> <Output filename>\n\n");
}
...
*/
        ...
        if (g_cType=='I') AppendPerfectDenEx(cChromosome,nOffset,nSNPCnt);
        else AppendPerfectDen(cChromosome,nOffset,nSNPCnt);
...
int main(int argc, char** argv) {
    ...
    for (int j=0;j<24; j++)
    {
       sprintf(acFile,"%s/chr%d_read_den",argv[2],j+1);
    ...
    g_cType = argv[3][0]; g_cSample = argv[4][0];
