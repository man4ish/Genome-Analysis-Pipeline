...
float fPEScore,fAvgQS,fRS,fReadDenScore,fSNPScore;
                          ...
                          int nRvsSNPCnt, int nSNPCnt, int nTotalRD, int nPECnt)
{
    ...
    int nMappableBases=unTotalCnt,nMappableReptDens=unReptDenVals,nReptDenScore,nStrandScore;
    float fSNPReadScore;

    fPEScore = !(nPECnt < g_fYCoverage)?100:(float)nPECnt*(100.00/g_fYCoverage);
    ...
    fReadDenScore = (fRS > 25.00)?100.00:fRS*4.00;
    ...
    fSNPScore = (fPEScore+fSNPReadScore+(float)nStrandScore+fReadDenScore+(float)nReptDenScore)/420.00;

    fprintf(g_pfOut,"%.2f\t%.2f\t%d\t%.2f\t%d\t%.4f\n",
                     fPEScore,fSNPReadScore,nStrandScore,fReadDenScore,nReptDenScore,fSNPScore);
}
