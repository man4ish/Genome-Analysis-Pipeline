...
void GenerateList()
{
    ...
    char acbuf[4096]; char *pChr=NULL; int nChromosome; stRecList *pRecList=NULL;
    ...
    while(!feof(g_pfFile1))
    {
        ...
        pChr = strtok(NULL,"\t"); pRecList->pData[pRecList->nUsed].nFwdSR = atoi(pChr);
        pChr = strtok(NULL,"\t"); pRecList->pData[pRecList->nUsed].nRvsSR = atoi(pChr);
        ...
        pChr = strtok(NULL,"\t"); pRecList->pData[pRecList->nUsed].nPEndCnt = atoi(pChr); 
        pChr = strtok(NULL,"\t"); pRecList->pData[pRecList->nUsed].fAvgQryPos = atof(pChr);
...
void AnalyseData()
{
    char acHeader[] = "Chr\tOffset\tAllele\tSNP_Density_1\tSNP_Density_2\t"
                      "Total_SNP_ Density\tRead_Density_1\tRead_Density_2\t"
                      "Total_Read_Density\tAVQS_1\tAVQS_2\tAVQS\tdbSNP\tRS\t"
                      "Fwd_SNP_1\tFwd_SNP_2\tTotal_Fwd_SNP_1\tRvs_SNP_1\t"
                      "Rvs_SNP_2\tTotal_Rvs_SNP_1\tPEnd_1\tPEnd_2\tTotal_PEnd\t"
                      "AVQryPos_1\tAVQryPos_2\tTotal_AVQryPos\n";

    fprintf(g_pfCommon,"%s",acHeader);
    fprintf(g_pfUnique1,"%s",acHeader); fprintf(g_pfUnique2,"%s",acHeader);

    fprintf(stdout,"Analysing Data ... \n");
    char acbuf[4096]; char *pChr=NULL; stRecList *pRecList=NULL; stData ostData;
    ...
    while(!feof(g_pfFile2))
    {
        ...
        pChr = strtok(NULL,"\t"); ostData.nFwdSR = atoi(pChr);
        pChr = strtok(NULL,"\t"); ostData.nRvsSR = atoi(pChr);
        pChr = strtok(NULL,"\t"); ostData.nSNPCnt = atoi(pChr);
        pChr = strtok(NULL,"\t"); ostData.nReadDensity = atoi(pChr);
        pChr = strtok(NULL,"\t"); ostData.nPEndCnt = atoi(pChr);  
        pChr = strtok(NULL,"\t"); ostData.fAvgQryPos = atof(pChr);
        ...
        {              
            ...
            if (ostData.nReadDensity < 1) nReadStrength = 101;
            ...
            fprintf(g_pfUnique2,"%d\t%s\t-\t%d\t%d\t-\t%d\t%d\t-\t%.2f\t%.2f\t%s\t%d\t"
                    ...
                    ostData.fAvgQScore,pChr,nReadStrength,ostData.nFwdSR,ostData.nFwdSR,
                    ostData.nRvsSR,ostData.nRvsSR,ostData.nPEndCnt,ostData.nPEndCnt,
                    ostData.fAvgQryPos,ostData.fAvgQryPos);
        }
        else{
            pRecList->pData[nIdx].bUnique = false;
            pRecList->pData[nIdx].nFwdSR2 = ostData.nFwdSR;
            pRecList->pData[nIdx].nRvsSR2 = ostData.nRvsSR;
            pRecList->pData[nIdx].nSNPCnt2 = ostData.nSNPCnt;
            pRecList->pData[nIdx].nReadDensity2 = ostData.nReadDensity;
            pRecList->pData[nIdx].nPEndCnt2 = ostData.nPEndCnt;
            pRecList->pData[nIdx].fAvgQryPos2 = ostData.fAvgQryPos;
       ...
       for (int j=0; j< pstRecList->nUsed; j++)        
       {
            ...
            if (pstRecList->pData[j].bUnique)
            {
                ...
                fprintf(g_pfUnique1,"%d\t%s\t%d\t-\t%d\t%d\t-\t%d\t%.2f\t-\t%.2f\t%s\t%d\t"
                        ...
                        pstRecList->pData[j].nFwdSR,pstRecList->pData[j].nFwdSR,
                        pstRecList->pData[j].nRvsSR,pstRecList->pData[j].nRvsSR,
                        pstRecList->pData[j].nPEndCnt,pstRecList->pData[j].nPEndCnt,
                        pstRecList->pData[j].fAvgQryPos,pstRecList->pData[j].fAvgQryPos);
            }
            else
            {
                ...
                fprintf(g_pfCommon,"%d\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t%.2f\t%.2f\t%.2f\t%s\t%d\t"
                        ...
                        pstRecList->pData[j].nFwdSR,pstRecList->pData[j].nFwdSR2,
                        pstRecList->pData[j].nFwdSR+pstRecList->pData[j].nFwdSR2,
                        pstRecList->pData[j].nRvsSR,pstRecList->pData[j].nRvsSR2,
                        pstRecList->pData[j].nRvsSR+pstRecList->pData[j].nRvsSR2,
                        pstRecList->pData[j].nPEndCnt,pstRecList->pData[j].nPEndCnt2,  
                        pstRecList->pData[j].nPEndCnt+pstRecList->pData[j].nPEndCnt2, 
                        pstRecList->pData[j].fAvgQryPos,pstRecList->pData[j].fAvgQryPos2,
                        (pstRecList->pData[j].fAvgQryPos+pstRecList->pData[j].fAvgQryPos2)/2); 
            }
        }
