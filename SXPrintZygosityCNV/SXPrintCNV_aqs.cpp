...
void ProcessRecs()
{
    ...
    while(!feof(g_pfIn))
    {
        ...
        if (acbuf[0] == 'C') { fprintf(g_pfOut,"%s\tMappable_Bases\tMappable_Read_Densities"
                                               "\tMappable_Repeat_Densities\tMappable_Bases_Repeat"
                                               "\tRepeat_Densities_MBP\n",acbuf); continue;}
