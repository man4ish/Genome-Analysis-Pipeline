                                 ...
                                 const char *pcGenomeVer,const char *pcOutputDir)
{
    ...
    m_bSNVLst = true;
    m_pcGenomeVer=pcGenomeVer; m_pcOutputDir=pcOutputDir;
}
...
void CSXReadSNV::PrintSNVAnalysis()
{
    ...
    fprintf(pf,"SNP total count\t%lu\n",m_stSNVAnalysis.ulTotalSNP);
    fprintf(pf,"INS total count\t%lu\n",m_stSNVAnalysis.ulTotalINS);
    fprintf(pf,"DEL total count\t%lu\n",m_stSNVAnalysis.ulTotalDEL);
...
void CSXReadSNV::PrintUnSortedSNV(stSNV *pSNV, unsigned long ulTotalRecs)
{
    ...
    {
        if (++ulTest1==100000){fprintf(stdout,"Total Unsorted Recs being printed = %lu X 1.0M",++ulTest2);ulTest1=0;}
...
void CSXReadSNV::PrintSortedSNV(stSNVRecList aSNVRecList[])
{
    ...
    for (unsigned long i=0; i<24; i++)
    {
        ...
        {
            if (++ulTest1==100000){fprintf(stdout,"Total Sorted Recs being printed = %lu X 1.0M",++ulTest2);ulTest1=0;}
                                   ...
                                   size_t &merge_list_alloc, size_t &merge_list_used)
{
    ...
    char cChromosome_Curr,cRefBase_Curr,cVarBase_Curr,cRef_ACGT=0,cVar_ACGT=0,cStrand;
...
void CSXReadSNV::PrintClusteredRecs(stSNVClusterRec **lpSNVClustered, size_t merge_list_used)
{
    ...
    {
         if (++ulTest1==100000) {fprintf(stdout,"Total Clustered Recs printed = %lu X 0.1M",++ulTest2);ulTest1=0;}  
...
Exit:
    ...
    for (int j=0;j<24; j++){if (m_apfRD[j]) fclose(m_apfRD[j]);}
    ...
    {        
        ...
        else if (m_pSNVClustered[i].cRefBase =='-' )
        {
          if (bINS1){
            ...
            if (m_pcdbIndel) bdbSNP=OutputINDELNovel(m_pSNVClustered[ChkPoly_INS.ulIdx].cChromosome,ChkPoly_INS.unOffset,
                                                     m_pSNVClustered[ChkPoly_INS.ulIdx].cVarBase, pfINS,acRsid);                                    else bdbSNP=m_pObjdbINS->OutputINDELNovel(m_pSNVClustered[ChkPoly_INS.ulIdx].cChromosome,ChkPoly_INS.unOffset,
...
EndINS:
	    ...
	    if (bDEL1){
            ...
            if (m_pcdbIndel) bdbSNP=OutputINDELNovel(m_pSNVClustered[ChkPoly_DEL.ulIdx].cChromosome,ChkPoly_DEL.unOffset,
                                                     m_pSNVClustered[ChkPoly_DEL.ulIdx].cVarBase, pfDEL,acRsid); 
            else bdbSNP=m_pObjdbDEL->OutputINDELNovel(m_pSNVClustered[ChkPoly_DEL.ulIdx].cChromosome,ChkPoly_DEL.unOffset,
                                                      m_pSNVClustered[ChkPoly_DEL.ulIdx].cVarBase, pfDEL,acRsid);
