...
bool CSXDBIndelgChecker::matchMNVINDEL(const char *pcAllele, int nAlleleLen, const char orient,const char* db)
{
     int nLen=0;
     ...
     for (unsigned int ii =0; ii< m_dbvec_INDEL.size();ii++) {
          nLen = strlen(&(m_dbvec_INDEL[ii][0]));
          
          for (int j=0; j < (nLen-nAlleleLen)+1; j++)
          { 
               if (memcmp(&(m_dbvec_INDEL[ii][j]),pcAllele,nAlleleLen)==0) return 1;
          } 
     }
...
bool CSXDBIndelgChecker::MapMNVAllele( Key* location ,const char *pcVariant, char acRsid[])
{
     m_bFound_INDEL = false;
     m_InDelret = m_dbInDelMap.equal_range(location);
     ...
     for (m_dbInDelit=m_InDelret.first; m_dbInDelit!=m_InDelret.second; ++m_dbInDelit)
     {
         if (matchMNVINDEL(pcVariant,strlen(pcVariant),(m_dbInDelit->second)->orient,(m_dbInDelit->second)->allele))
         {
             if (!m_bFound_INDEL){ m_bFound_INDEL = true; sprintf(acRsid,"%s",(m_dbInDelit->second)->rsid); }
             else { strcat(acRsid,","); strcat(acRsid,(m_dbInDelit->second)->rsid); }
         }
...
bool CSXDBIndelgChecker::OutputMNVINDELNovel(int chrno, size_t offset,const char *pcVariant,
                                             FILE* pf, char acRsid[])
{
    m_bdbINDEL = false; Key * snpkey = new Key(chrno, offset);
    dbmap::const_iterator elefound = m_dbInDelMap.find (snpkey);
    ...
    if (elefound != m_dbInDelMap.end ()) {
        ...
        if (MapMNVAllele(snpkey,pcVariant,acRsid)) m_bdbINDEL = true;
        else sprintf(acRsid,"-");
    }
    else sprintf(acRsid,"-");
...
bool CSXDBIndelgChecker::GenerateDBINDELMap(FILE *dbfile)
{
    ...
    while (fgets(dbline, sizeof(dbline), dbfile))
    {
        ...
        while(pch != NULL){
          lpc[i++]=pch; if (i>11) break; pch = strtok (NULL,"\t");
        }
        ...
        for (int i = atoi(lpc[1]); i < atoi(lpc[2])+1; i++)
        {
                Key * key = new Key(chr_num, i);                       // Key(int _chronum, size_t  _offset):chronum(_chronum),offset(_offset)
                Value * value = new Value(lpc[8],lpc[3][0], lpc[5]);   // Value(const char* _rsid, const char * _orient, const char * _allele)
                m_dbInDelMap.insert(pair<Key *, Value *>(key, value));
        }
