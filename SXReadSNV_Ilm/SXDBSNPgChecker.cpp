...
bool matchMNV(const char *pcMNVAllele,int nAlleleLen, const char orient,const char* db)
{
     g_dbvec.clear();
    ...
    for (unsigned int ii =0; ii< g_dbvec.size();ii++) {
        if (memcmp(&(g_dbvec[ii][0]),pcMNVAllele,nAlleleLen)==0) return 1;
    }
...
inline bool MapMNVAllele( Key* location, const char *pcVariant, char acRsid[])
{
     g_bFound = false; g_SNPret = g_dbSNPMap.equal_range(location);

     for (g_dbSNPit=g_SNPret.first; g_dbSNPit!=g_SNPret.second; ++g_dbSNPit)
     {
         if (matchMNV(pcVariant,strlen((char*)pcVariant),(g_dbSNPit->second)->orient,(g_dbSNPit->second)->allele))
         {
             if (!g_bFound){ g_bFound = true; sprintf(acRsid,"%s",(g_dbSNPit->second)->rsid); }
             else { strcat(acRsid,","); strcat(acRsid,(g_dbSNPit->second)->rsid); }
         }
...
bool OutputMNPNovel(int chrno, size_t offset, const char *pcVariant,
                    FILE* pf, char acRsid[])
{
    g_bdbSNP = false; Key * snpkey = new Key(chrno, offset);
    dbmap::const_iterator elefound = g_dbSNPMap.find (snpkey);
    ...
    if (elefound != g_dbSNPMap.end ()) {
        if (MapMNVAllele(snpkey,pcVariant,acRsid)) g_bdbSNP = true; else sprintf(acRsid,"-");
    }
    else sprintf(acRsid,"-");
...
bool GenerateDBSNPMap( FILE *dbfile)
{
    ...
    char dbline[40960], *pch, *lpc[12]; int i, chr_num;
    ...
    while (fgets(dbline, sizeof(dbline), dbfile))
    {
        ...
        while(pch != NULL){
	  lpc[i++]=pch; if (i>22) break; pch = strtok (NULL,"\t");
        }
