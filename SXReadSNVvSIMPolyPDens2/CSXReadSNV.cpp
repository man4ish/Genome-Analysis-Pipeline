...
void CSXReadSNV::setAvgQSLstInputs(const char *pcdbSnp, const char *pcdbIndel,
                                   ...
                                   int nQScoreOfs,const char *pcHsRefFilePath,const char *pcAmbiguityFile)
{
    ...
    m_pcAmbiguityFile=pcAmbiguityFile; m_bAvgQScoreLst=true;
}
                                   ...
                                   size_t &merge_list_alloc, size_t &merge_list_used)
{
    ...
    for (unsigned long i=0; i<ulTotalRecs; i++)
    {
           fprintf(pfIn,"%d\t%u\t%c\t%c\t%d\t%d\t%d\t%d\n",
                pSNV[i].cChromosome,ComputeOffset(pSNV[i].acOffset),
                toupper(pSNV[i].cRefBase),toupper(pSNV[i].cVarBase),
    ...
    */
    ...
    for (unsigned long i=0; i<ulTotalRecs; i++)
    {
        nChro = abs(pSNV[i].cChromosome); pSNVRecList = &aSNVRecList[nChro-1];
    ...
    */
    ...
    (*lpSNVClustered)[merge_list_used].cChromosome = cChromosome_Curr;
   ...
   for (int i=0; i < 24; i++)
   {
    ...
    /*    
    char acClusterFileName[1024]; sprintf(acClusterFileName,"clustered_%s.txt",m_pcSampleID);        
    FILE *pfCluster=fopen(acClusterFileName,"w");

    for (unsigned long i=0; i < merge_list_used+1; i++)
    {
        fprintf(pfCluster,"%d\t%u\t%c>%c\t%u\t%u\t%u\t%u\n",
                (*lpSNVClustered)[i].cChromosome,(*lpSNVClustered)[i].unOffset,
...
inline void CSXReadSNV::GenerateSNVTbl()
{
    ...
    */
    ...
    m_pSNVClustered[m_merge_list_used].cChromosome = cChromosome_Curr;
    m_pSNVClustered[m_merge_list_used].unOffset = unOffset_Curr;
    m_pSNVClustered[m_merge_list_used].cRefBase = cRefBase_Curr;
    m_pSNVClustered[m_merge_list_used].cVarBase = cVarBase_Curr;
    m_pSNVClustered[m_merge_list_used].ulTotalQP = usQP;  
    m_pSNVClustered[m_merge_list_used].ulTotalQS = usQS;
    m_pSNVClustered[m_merge_list_used].ucMaxQS = usQS;
    m_pSNVClustered[m_merge_list_used].unCnt = 1;
    m_pSNVClustered[m_merge_list_used].bKeep = true;

    if (usPEnd != 0) m_pSNVClustered[m_merge_list_used].unPEndCnt = 1;

    if (cStrand==0) m_pSNVClustered[m_merge_list_used].unRvsCnt = 1;
    else if (cStrand==1) m_pSNVClustered[m_merge_list_used].unFwdCnt = 1;
              
    if ((cRefBase_Curr!='A' && cRefBase_Curr!='C' && cRefBase_Curr!='G' && cRefBase_Curr!='T' && 
         cRefBase_Curr!='-')|| (cVarBase_Curr!='A' && cVarBase_Curr!='C' && cVarBase_Curr!='G' &&
         cVarBase_Curr!='T' && cVarBase_Curr!='-'))
    {
        m_pSNVClustered[m_merge_list_used].bKeep=false;
   ...
   for (int i=0; i < 24; i++)
   {
       ...
       {   
               ...
               cRefBase_Curr != cRef_ACGT||cVarBase_Curr != cVar_ACGT) 
           {
               ...
               if (m_merge_list_used == m_merge_list_alloc) AllocMergeListSize();
          ...
          else
          {
               ...
               if (m_pSNVClustered[m_merge_list_used].ucMaxQS < usQS)
                   m_pSNVClustered[m_merge_list_used].ucMaxQS = usQS;
                 
               //if (m_pSNVClustered[m_merge_list_used].sMaxPEnd < usPEnd)
               //    m_pSNVClustered[m_merge_list_used].sMaxPEnd = usPEnd;

               m_pSNVClustered[m_merge_list_used].unCnt++;

               if (usPEnd != 0) m_pSNVClustered[m_merge_list_used].unPEndCnt++;    

               if (cStrand==0) m_pSNVClustered[m_merge_list_used].unRvsCnt++;
               else if (cStrand==1) m_pSNVClustered[m_merge_list_used].unFwdCnt++;   
          }
...
//fprintf(stdout,"Chro = %d\tOffset=%lu\n",m_pSNVClustered[0].cChromosome, m_pSNVClustered[0].unOffset);
...
unsigned long g_ulFnd=0,g_ulFndSNP=0;
...
//fprintf(stdout,"unOffset=%u\tulStart=%u\tulEnd=%u\tm_pSNVClustered[ulSearch].unOffset=%u\n",
//        unOffset,ulStart,ulEnd,m_pSNVClustered[ulSearch].unOffset);

       if (m_pSNVClustered[ulSearch].unOffset == unOffset){
           if (m_pSNVClustered[ulSearch].cRefBase!='-' && m_pSNVClustered[ulSearch].cVarBase!='-') g_ulFndSNP++;
           g_ulFnd++; 
...
void CSXReadSNV::ProcAmbiguityList()
{
    ...
    unsigned long ulIdx=0,j=0,ulCnt,ulStart=0,ulEnd,ulFoundCnt=0,ulFoundSNP=0; 
    ...
    for (int i=0; i<24; i++)
    {
        ...
        {                             
            ...
            if (SearchClusteredList(ulStart,ulEnd,pAmbiguityClustered[ulIdx].unOffset,ulFound))
            {
                  for (j=ulFound-10; j < ulFound+10; j++)
                  {
                       ...
                       )                          
                       {
                           ...
                           ulFoundCnt++;
                           if (m_pSNVClustered[j].cRefBase!='-' && m_pSNVClustered[j].cVarBase!='-')  
                           {
                              ulFoundSNP++;
                              fprintf(pfMergedList,"%d\t%u\t%c>%c\t%u\t%u\n",
                              pAmbiguityClustered[ulIdx].cChromosome,pAmbiguityClustered[ulIdx].unOffset,
            ...
            if (!bMerged){
                fprintf(pfList,"%d\t%u\t%c>%c\t%u\t%u\t%u\t%u\n",
                pAmbiguityClustered[ulIdx].cChromosome,pAmbiguityClustered[ulIdx].unOffset,
                pAmbiguityClustered[ulIdx].cRefBase,pAmbiguityClustered[ulIdx].cVarBase,
...
void CSXReadSNV::OutputAvgQScoreTable()
{
    ...
    for (int i=0; i<24;i++)
    {
        sprintf(acFile,"%s/Chr%02d_read_den_perfect",m_pcDensFilePath,i+1);
    ...
    fprintf(stdout,"Process Ambiguity List....\n"); ProcAmbiguityList(); 
    fprintf(stdout,"Filtering SNV Table...\n"); FilterSNVTbl(m_merge_list_used);
    fprintf(stdout,"Load Human Reference Sequence ...\n"); LoadHRefSeq();

/*
    char acClusterFileName[1024]; sprintf(acClusterFileName,"AfterFilterSNV_%s.txt",m_pcSampleID);        
    FILE *pfCluster=fopen(acClusterFileName,"w");

    for (unsigned long i=0; i < m_merge_list_used+1; i++)
    {
        fprintf(pfCluster,"%d\t%u\t%c>%c\t%u\t%u\t%u\t%u\t%s\n",
                m_pSNVClustered[i].cChromosome,m_pSNVClustered[i].unOffset,
                m_pSNVClustered[i].cRefBase,m_pSNVClustered[i].cVarBase,
                m_pSNVClustered[i].unCnt,m_pSNVClustered[i].ulTotalQS,
                m_pSNVClustered[i].ulTotalQP,m_pSNVClustered[i].ucMaxQS,
                m_pSNVClustered[i].bKeep?"Keep":"Throw");
    }
...
int CSXReadSNV::CheckPoly(int nChroIdx, unsigned int unOffset, char cAllele,eSNVType eSNV,FILE *pf, unsigned int unRDCnt)
{
    ...
    if (m_apcHsRef[nChroIdx][unOffset]!=cAllele) { 
       unOffset++; //if (eSNV == eDEL)unOffset++; 
         
       //fprintf(pf,"%d\t%c\t%u\n",unOffset,cAllele,unRDCnt);
       return unOffset;
    }
...
/////Testing///////////////////////////////
    if (unOffset != i-1){
       ...
       //fprintf(pf,"+%d\t%c\t%s\t%u\n",i,cAllele,acAllele,unRDCnt);      
    }
    else
    {       
       //fprintf(pf,"*%d\t%c\t%c\t%u\n",i,cAllele,m_apcHsRef[nChroIdx][unOffset],unRDCnt);
    }  
...
/////////////////////////////
    ...
    char acPDensFName[1024]; sprintf(acPDensFName,"PDens_del_%s.lst",m_pcSampleID);
    FILE *pfPDenDEL = fopen(acPDensFName,"w"); 
 
    sprintf(acPDensFName,"PDens_ins_%s.lst",m_pcSampleID);
    ...
    for (unsigned long i=0; i < (m_merge_list_used+1); i++)
    {
        ...
        else if (m_pSNVClustered[i].cRefBase =='-' )
        {
            if (bINS1){
                ...
                ChkPoly_INS.unOffset=CheckPoly(unChroIdx,m_pSNVClustered[i].unOffset,
                                               m_pSNVClustered[i].cVarBase,eINS,pfPolyINS,m_pSNVClustered[i].unCnt);
            ...
            if (ChkPoly_INS.bPoly){
               OutputChrom(pfPolyINS,m_pSNVClustered[ChkPoly_INS.ulIdx].cChromosome);
               fprintf(pfPolyINS,"\t%u\n",ChkPoly_INS.unOffset);
            }

            OutputChrom(pfPDenINS,m_pSNVClustered[ChkPoly_INS.ulIdx].cChromosome);
...
EndINS:
	    ...
	    if (bDEL1){
                ...
                ChkPoly_DEL.unOffset = CheckPoly(unChroIdx,m_pSNVClustered[i].unOffset,
                                                 m_pSNVClustered[i].cRefBase,eDEL,pfPolyDEL,m_pSNVClustered[i].unCnt);

                ChkPoly_DEL.unReadDensity = GetReadDensity(m_apfRD[unChroIdx], ChkPoly_DEL.unOffset); 
	    ...
	    unReadDensity = ChkPoly_DEL.unReadDensity + m_pSNVClustered[ChkPoly_DEL.ulIdx].unCnt;
            ...
            if (ChkPoly_DEL.bPoly){
               OutputChrom(pfPolyDEL,m_pSNVClustered[ChkPoly_DEL.ulIdx].cChromosome);
               fprintf(pfPolyDEL,"\t%u\n",ChkPoly_DEL.unOffset);
            }

            OutputChrom(pfPDenDEL,m_pSNVClustered[ChkPoly_DEL.ulIdx].cChromosome);   
