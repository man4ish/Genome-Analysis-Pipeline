...
{
    printf("Synamatix SXPrintStatsINS Copyright 2011 Synamatix Sdn Bhd\n");
    printf("Built %s %s\n", __DATE__, __TIME__);
    printf("Usage: %s\n", argv[0]);
...
void ProcessInputFile()
{
     ...
     while (!feof(g_pfIn)){
         ...
         pChr = strtok(NULL,"\t"); //Inserted_Base	
         cAllele=pChr[0];
...
void OutputStats()
{
    ...
    fprintf(g_pfOut,"\n");
    fprintf(g_pfOut,"Inserted Bases\tTotal INS-Count\tTotal INS-Percentage\tNovel INS\t"
                    "Known INS\tINS in Gene\tINS in Exon\t"
                    "INS in Intron\tINS in Promoter\tINS in miRNA\tINS in UTR\n");
    ...
    fprintf(g_pfOut,"Confident Score Distribution\tINS Count\tPercentage\tPercentage (cumulative)\n"); 
