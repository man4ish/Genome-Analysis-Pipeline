...
typedef struct _stKey{    
    ...
    ~_stKey()
    {
        if (pcRefBase) delete[] pcRefBase;  
        if (pcVarBase) delete[] pcVarBase;  
    }     
} stKey;
...
typedef struct _compare{
    ...
    {
        if (p1->unGiNum != p2->unGiNum) return p1->unGiNum < p2->unGiNum;

        if (p1->unOffset != p2->unOffset) return p1->unOffset < p2->unOffset;

        if (strcmp(p1->pcRefBase,p2->pcRefBase)!=0) return strcmp(p1->pcRefBase,p2->pcRefBase) < 0;

        return strcmp(p1->pcVarBase,p2->pcVarBase) < 0;
    }
};
