...
//#define INIT_MERGED_LIST_SIZE 5000
//#define EXT_MERGED_LIST_SIZE  1000
...
public:
    ...
    void setSNVLstInputs(const char *pcGene, const char *pcGeneKeyword,
...
private:
    ...
    void LogInvalidRec(FILE *pf, stSNVRec &Rec);

    bool LoadGeneKeyword(FILE *pf);
    void PrintGeneKeyword(FILE *pf);
    ...
    inline unsigned int GetReadDensityEx(FILE *pf, unsigned int uioffset, unsigned int unFranking);
    ...
    GENE_KEYWORD_LIST m_GeneKeywordList;
    GENE_KEYWORD_LIST::iterator m_GeneKeywordItr;
    ...
    const char* m_pcGene;
    const char* m_pcGeneKeyword;
    const char* m_pcExon;
    const char* m_pcdbSnp;
    const char *m_pcdbIndel;
    const char* m_pcGeneName;
    const char* m_pcCNV;
