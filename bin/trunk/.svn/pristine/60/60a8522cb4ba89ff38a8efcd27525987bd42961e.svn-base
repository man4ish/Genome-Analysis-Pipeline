...
inline bool MapAllele( Key* location ,const char &variant, char acRsid[])
{
     ...
     for (g_dbInDelit=g_InDelret.first; g_dbInDelit!=g_InDelret.second; ++g_dbInDelit)
     {
         ...
         if (acRsid[0]==0) sprintf(acRsid,"%s",(g_dbInDelit->second)->rsid);
         else { strcat(acRsid,","); strcat(acRsid,(g_dbInDelit->second)->rsid); }         
     }
     return g_bFound_INDEL;
}
...
bool matchMBINDEL(const char *pcVar ,const char orient,const char* db)
{
     g_dbvec_INDEL.clear();
     ...
     for (unsigned int ii =0; ii< g_dbvec_INDEL.size();ii++) {
          ...
          if (strcmp(g_dbvec_INDEL[ii].c_str(),pcVar)==0) return 1;
     }
...
inline bool MapMBIndelAllele( Key* location ,const char *pcVar, char acRsid[])
{
     g_bFound_INDEL=false; acRsid[0]=0; g_InDelret=g_dbInDelMap.equal_range(location);

     for (g_dbInDelit=g_InDelret.first; g_dbInDelit!=g_InDelret.second; ++g_dbInDelit)
     {
/*
         if (matchMBINDEL(pcVar,(g_dbInDelit->second)->orient,(g_dbInDelit->second)->allele))
         {
             if (!g_bFound_INDEL){ g_bFound_INDEL = true; sprintf(acRsid,"%s",(g_dbInDelit->second)->rsid); }
             else { strcat(acRsid,","); strcat(acRsid,(g_dbInDelit->second)->rsid); }
         }
...
bool OutputMBINDELNovel(int chrno, size_t offset,const char *pcVar, FILE* pf, char acRsid[])
{
    g_bdbINDEL = false; Key * snpkey = new Key(chrno, offset);
    dbmap::const_iterator elefound = g_dbInDelMap.find (snpkey);

    if (elefound != g_dbInDelMap.end ()) {
        if (MapMBIndelAllele(snpkey,pcVar,acRsid)) g_bdbINDEL = true; else sprintf(acRsid,"-");
