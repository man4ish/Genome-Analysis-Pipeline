...
void ProcessRecs()
{
    ...
    {        
        ...
        fPEScore = !(nPECnt < 15)?100.00:(float)nPECnt*(100.00/15.00);
        fSNPReadScore = !(nSNPCnt < 30.00)?100.00:(float)nSNPCnt*(100.00/30.00);

        if (!(nFwdSNPCnt < 15) && !(nRvsSNPCnt < 15)) fStrandScore = 100.00;
        else if ((nFwdSNPCnt < 2) && (nRvsSNPCnt < 2)) fStrandScore = 0.00; 
        else {
           nMin = (nFwdSNPCnt < nRvsSNPCnt)? nFwdSNPCnt:nRvsSNPCnt;
           fStrandScore = (float)nMin*(100.00/15.00);                    
        } 
        ...
        fAQSScore = (fAvgQS < 15.00)?0:(fAvgQS-15.00)*100/15;
