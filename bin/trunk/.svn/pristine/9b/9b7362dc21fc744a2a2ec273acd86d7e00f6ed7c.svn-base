...
public class CGZygosity_SingleSample {
	...
	public static void main(String arg[]) throws EOFException{		
		...
		if (arg.length < 5){
			...
			System.out.format("CGZygosity\n");
			System.out.format("\t<Input filename> <Min. Supporting Reads> <Sample :- 1 or 2> " + 
                                          " <Diploid Probability Param:- 1 to 5> <Output filename>\n\n");
		    return;
		}
        ...
        DiploidProbabilityParam = Integer.parseInt(arg[3]);  
		...
		try{
			...
			FileOutputStream file_output = new FileOutputStream(arg[4]);				
			...
			while (true){
                ...
                //if (arg[2].equals("1")){//DeletionDataSplit[3].equals("-")){ 
                	delCount = Integer.parseInt(DeletionDataSplit[5]); //[4]);                	
                	AQS = Double.parseDouble(DeletionDataSplit[9]);    //[10]);                	
                //}
                //else {
                // 	delCount = Integer.parseInt(DeletionDataSplit[4]); //[3]);                	
                //	AQS = Double.parseDouble(DeletionDataSplit[10]);   //[9]);                	                	
                //}                
                 
                TotReadCount = Integer.parseInt(DeletionDataSplit[DeletionDataSplit.length-1]); //14]); //perfect densities
                ...
                strOut = "";
                for (int i=0; i < (DeletionDataSplit.length-1); i++)
                    strOut += DeletionDataSplit[i]+"\t";      
                    
                if (delCount < Integer.parseInt(arg[1])){ 
                	greyareaCount++; strOut += "-\n"; //DeletionData + "\t-\n"; greyareaCount++;
                }
                    ...
                    if(RS<0.7){ heteroCount++; strOut += "het\n";} //DeletionData + "\thet\n"; }
                    else{ homoCount++; strOut += "hom\n";} //DeletionData + "\thom\n"; }
                }			       
