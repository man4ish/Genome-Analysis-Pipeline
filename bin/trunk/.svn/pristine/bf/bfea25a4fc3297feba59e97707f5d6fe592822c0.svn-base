...
static void banner(char *argv[])
{
    ...
    printf("Built %s %s\n", __DATE__, __TIME__);
    printf("Usage: \n", argv[0]);
    printf("\t<Seq filename> <SNP list filename> <INS list filename> <DEL list filename> <Output filename> <Size>\n\n");
}
...
void GetSNVSeqVariant(int nChro, int nStart, int nEnd, const char* pcSeq, stSeqSNV &ostSeq)
{
...
/*
if (nEnd == 23023232 && nSNV == eINS){
       printf("nBp=%d\tOffset=%d\tnIdx=%d\t",(nBp-nStart)+1,nBp,nIdx);
}
*/
              if (nIdx==-1) continue;
...
/*
if (nEnd == 23023232 && nSNV == eINS){
   printf("%c\t",pstRec[nIdx].acAllele[2]);
} 
*/ 
                   }                                       
                   else if (nSNV == eINS) {
...
if (nEnd == 23023232){
   printf("[");
   for (int i=0; i < astSeqSNV[1].nUsed; i++)
       printf("%c",astSeqSNV[1].pcData[i]);
   printf("]\n");
...
/*
         ...
         if (astSeqSNV[eINS].nUsed > 0){
             ...
             ostSeq.Add(astSeqSNV[eINS].pcData,astSeqSNV[eINS].nUsed);             
         }

         if (astSeqSNV[eDEL].nUsed > 0) {
            if (astSeqSNV[eSNP].nUsed > 0||astSeqSNV[eINS].nUsed > 0){ostSeq.Add(',');} 
...
//}
        ...
        pChr = strtok(NULL,"\t");   
        GetSNVSeqVariant(nChro, nOffset-g_nSize, nOffset, pChr, ostSeq);   
        ostSeq.Add('\0'); fprintf(g_pfOut,"%s\t",ostSeq.pcData);  
         
        pChr = strtok(NULL,"\t");
        GetSNVSeqVariant(nChro, nOffset, nOffset+g_nSize, pChr, ostSeq);
        ostSeq.Add('\0');
...
{
    if (argc != 7){ printf("Invalid parameters\n");banner(argv); exit(9);}
