...
typedef struct _stACGT
{
    stVal *pVal[4];

    _stACGT()
    {
        pVal[0]=pVal[1]=pVal[2]=pVal[3]=NULL;
    }
...
typedef struct _stDEL
{
    unsigned int unDEL;
    unsigned int ulDELTotalQS;
    //unsigned char ucMinQP;
    //unsigned char ucMaxQP;
    //unsigned int ulSumQP;
...
typedef struct _stSNVTblKey{
    ...
    _stSNVTblKey(stSNV *pSNV)
    {
        cChromosome = abs(pSNV->cChromosome);
        unOffset = ComputeOffset(pSNV->acOffset);
        cRefBase = toupper(pSNV->cRefBase);
        cVarBase = toupper(pSNV->cVarBase);
    }
...
typedef struct _stSNVClusterRec{
    ...
    {
        bKeep = true;
        ulTotalQS=0; ucMaxQS=0; sMaxPEnd=0;
    }
}stSNVClusterRec;
...
typedef struct _compareSNVTblKey{
    bool operator()(const stSNVTblKey *st1, const stSNVTblKey *st2)
    {
        if (st1->cChromosome != st2->cChromosome)
            return st1->cChromosome < st2->cChromosome;
