...
void ProcessRecs()
{
    ...
    int nSNPReadScore,nStrandScore,nReptDenScore;  
    float fPEScore,fAvgQS,fRS,fReadDenScore,fAQSScore,fSNPScore;  
    ...
    {        
        ...
        fAQSScore = (fAvgQS < 15.00)?0.00:(fAvgQS-15.00)*(100.00/15.00);
        fSNPScore = (fPEScore+(float)nSNPReadScore+(float)nStrandScore+fReadDenScore+(float)nReptDenScore+fAQSScore)/520.00;
        
        fprintf(g_pfOut,"%s\t%.2f\t%d\t%d\t%.2f\t%d\t%.2f\t%.4f\n",
                        acOut,fPEScore,nSNPReadScore,nStrandScore,fReadDenScore,nReptDenScore,fAQSScore,fSNPScore);
    }
}
