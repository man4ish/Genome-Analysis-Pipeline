...
void ProcessRecs()
{
    ...
    {        
        ...
        fSNPReadScore = !(nSNPCnt < g_fYCoverage)?100:nSNPCnt*(100.00/g_fYCoverage); //(nSNPCnt > 4)?100:nSNPCnt*20;
        nStrandScore = (!(nFwdSNPCnt < g_fXCoverage) && !(nRvsSNPCnt < g_fXCoverage))?100:0; //((nFwdSNPCnt > 1) && (nRvsSNPCnt > 1))?100:0;  
        fRS = (float)nSNPCnt/(float)nTotalRD*100.00;
        fReadDenScore = (fRS > 20.00)?100.00:fRS*5.00; //(fRS > 10.00)?100.00:fRS*10.00;
