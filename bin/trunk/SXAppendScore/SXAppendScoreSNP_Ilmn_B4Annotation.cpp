...
{   
    printf("Synamatix SXAppendScoreSNP_Ilmn_B4Annotation Copyright 2010 Synamatix Sdn Bhd\n");
    printf("Built %s %s\n", __DATE__, __TIME__);
    printf("Usage: %s\n", argv[0]);
...
void ProcessRecs()
{
    int nSNPCnt,nPECnt,nTotalRD,nMappableBases,nMappableReptDens;
    int nReptDenScore;  
    float fPEScore,fSNPReadScore,fAvgQS,fRS,fReadDenScore,fAQSScore,fSNPScore;  
    float fYCoverage=g_fMean, fZCoverage=g_fMean/10.00;
    ...
    {        
        ...
        fPEScore = 0.00;//!(nPECnt < fXCoverage)?100.00:(float)nPECnt*(100.00/fXCoverage);
        fSNPReadScore = !(nSNPCnt < fYCoverage)?100.00:(float)nSNPCnt*(100.00/fYCoverage);

        if (!(fFwdSNPCnt < fZCoverage) && !(fRvsSNPCnt < fZCoverage))
            fStrandScore = 100;
        else{
            if (fFwdSNPCnt >  fZCoverage) fFwdSNPCnt = fZCoverage;
            if (fRvsSNPCnt >  fZCoverage) fRvsSNPCnt = fZCoverage;
            fStrandScore = ((fFwdSNPCnt+fRvsSNPCnt)*100)/(fZCoverage*2);
        }
        ...
        fAQSScore = (fAvgQS < 15.00)?0:(fAvgQS-15.00)*(100.00/25.00);
