...
static void banner(char *argv[])
{
    ...
    printf("Built %s %s\n", __DATE__, __TIME__);
    printf("Usage: %s\n", argv[0]);
    printf("\t<Input filename> <Mean> <Read Density files path> "
                          ...
                          int nRvsSNPCnt, int nSNPCnt, int nTotalRD, int nPECnt)
{
    ...
    int nMin,nMappableBases=unTotalCnt,nMappableReptDens=unReptDenVals,nReptDenScore;

    fPEScore = !(nPECnt < g_fXCoverage)?100.00:(float)nPECnt*(100.00/g_fXCoverage);
    fSNPReadScore = !(nSNPCnt < g_fYCoverage)?100.00:(float)nSNPCnt*(100.00/g_fYCoverage);

    if (!(nFwdSNPCnt < g_fXCoverage) && !(nRvsSNPCnt < g_fXCoverage)) fStrandScore = 100.00;
    else if ((nFwdSNPCnt < 2) && (nRvsSNPCnt < 2)) fStrandScore = 0.00;
    else {
         nMin = (nFwdSNPCnt < nRvsSNPCnt)? nFwdSNPCnt:nRvsSNPCnt;
         fStrandScore = (float)nMin*(100.00/g_fXCoverage);
    }
    ...
    fSNPScore = (fPEScore+fSNPReadScore+fStrandScore+fReadDenScore+(float)nReptDenScore)/420.00;

    fprintf(g_pfOut,"%.2f\t%.2f\t%.2f\t%.2f\t%d\t%.4f\n",
                     fPEScore,fSNPReadScore,fStrandScore,fReadDenScore,nReptDenScore,fSNPScore);
