...
#include "SXPrintZygosityCNVEx.h"


static void banner(char *argv[])
{
    printf("Synamatix SXPrintZygosityCNVEx Copyright 2010 Synamatix Sdn Bhd\n");
    printf("Built %s %s\n", __DATE__, __TIME__);
    printf("Usage: %s\n", argv[0]);
    printf("\t<Input filename> <CutOff> <Min. Supporting Reads for het.> "
           "<Min. Supporting Reads for hom.> <Means> <Read Density files path> "
           "<Repeat Density files path> <Masked Density files path> <SampleID> <Output filename>\n\n");
}
...
int main(int argc, char** argv) {
    ...
    */
    ...
    for (int j=0;j<24; j++)
    {
       sprintf(acFile,"%s/%s.seq%d.read_den_all",argv[6],argv[9],j+1);
       ...
       if (g_stReadDen[j].pcData==(void*)-1){
          fprintf(stdout, "Error: %s mmap failed: %s\n",acFile, strerror(errno));goto Err_Exit; 
       }  
    ...
    for (int j=0;j<24; j++)
    {
       sprintf(acFile,"%s/%s.seq%d.rept_den",argv[7],argv[9],j+1);
