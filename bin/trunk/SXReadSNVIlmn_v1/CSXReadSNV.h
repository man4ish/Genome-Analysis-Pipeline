...
typedef struct _stMBSNVRecList
{   
    stMBSNVRec *pMBSNVRec;
    int nTotal;
    int nUsed;
    pthread_t ThreadID;
    ...
    _stMBSNVRecList()
   { 
       pMBSNVRec = (stMBSNVRec*)malloc(sizeof(stMBSNVRec)*SNVREC_SIZE);
       nUsed = 0;
       nTotal = SNVREC_SIZE;  
   }
...
public:
    ...
    void setMBAvgQSLstInputs(const char *pcdbSnp, const char *pcdbIndel,
                             const char *pcDensFilePath,
                             unsigned int unMinSupportingReads,
                             unsigned int unMaxSupportingReads,
                             float fMinReadStrength,int nMinQScore,
                             const char *pcSampleID, const char *pcGenomeVer,int nQScoreOfs);

    void setMBSNVLstInputs(const char *pcBinnedFile,const char *pcExceptionFile,
                         const char *pcAnnotateFile,
                         unsigned int unMinSupportingReads,
                         unsigned int unMaxSupportingReads,
                         char cSNVType,const char *pcSampleID,
...
private:
    ...
    void FilterSNVRec(stSNVClusterRec &Rec1, stSNVClusterRec &Rec2, stSNVClusterRec &Rec3);
    void PrintBaseRepeat(FILE *pf,stSNVClusterRec &Rec);
    void LogInvalidRec(FILE *pf, stSNVRec &Rec, int nChromosome);
    void SetPEndnStrandInfo(stSNVClusterRec&, short unsigned int&, char&);
    ...
    void SetMBMiscInfo(stMBSNVClusterRec &MBSNVClusterRec, unsigned short &usPEnd, char &cStrand,
                       unsigned char &ucQryPos, char &cPEType, char &cAmbiguity, 
                       unsigned long &ulLnkFileOffset);

    void ReadRawMBSNVData(FILE *pf);
    void SortMBSNVData();
    void ClusterMBSNVData();
    void PrintMBAvgQScoreList(); 
    void OutputMBSNVList();
    void PrintMBSNPList(FILE *pfSrc);
    void PrintMBINSList(FILE* pfSrc);
    void PrintMBDELList(FILE* pfSrc); 


    inline void GenerateSNVTbl();
    inline void FilterSNVTbl();
    inline void GenerateMBSNVTbl(FILE *pfIn);      
    ...
    inline void AllocMergeMBListSize();
    ...
    stMBSNVRecList m_aMBSNVRecList[24]; 
    stMBSNVClusterRec *m_pMBSNVClustered;
    ...
    size_t merge_list_alloc, merge_list_used, merge_MBlist_alloc, merge_MBlist_used;
