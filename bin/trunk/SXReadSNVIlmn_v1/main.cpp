...
static void banner(char *argv[])
{
    ...
    printf("\t-f <Input file name>\n"
           ...
           " <Sample-ID> <Genome version eg. 37.1>\n"
           ...
           " <Genome version eg. 37.1> <Quality Score Offset>\n"
           "\t-gML <Binned Annotatation filename> <Binned Exception Annonatation filename> <Annotation filename>"
           " <Min Supporting Reads to be printed> <Max Supporting Reads to be printed> <SNV Type eg. S,I,D>"
           " <Sample-ID> <Genome version eg. 37.1>\n"
           "\t-gMLa <dbSNP database> <dbINDEL database> <Density Files' Path> <Min Supporting Reads>"
           " <Max Supporting Reads> <Min Read Stregth> <Min Quality Score> <Sample-ID>"
           " <Genome version eg. 37.1> <Quality Score Offset>\n"
    ...
    for (int i = 1; i < argc; i++)
    {
		...
		banner(argv); exit(9);
            ...
            pObj->setSNVLstInputs(pcBinnedFile,pcExceptionFile,pcAnnotateFile,unMinSupportingReads,
                                  unMaxSupportingReads,cType,pcSampleID,pcGenomeVer);
            continue;
	}
		...
		banner(argv); exit(9);
            ...
            pObj->setAvgQSLstInputs(pcdbSnp,pcdbIndel,pcDensFilePath,unMinSupportingReads,
                                    unMaxSupportingReads,fMinReadStrength,nMinQScore,pcSampleID,
                                    pcGenomeVer,nQScoreOfs);
            continue;
	}
        
        if (strcmp(argv[i], "-gML") == 0) {
           if (i >= argc - 8) {
                fprintf(stderr, "Invalid Parameters.\n");
                banner(argv); exit(9);
            }
            ...
            pObj->setMBSNVLstInputs(pcBinnedFile,pcExceptionFile,pcAnnotateFile,unMinSupportingReads,
                                    unMaxSupportingReads,cType,pcSampleID,pcGenomeVer);
            continue;
        }
    
        if (strcmp(argv[i], "-gMLa") == 0) {
           ...
           if (i >= argc - 9) {
            ...
            pObj->setMBAvgQSLstInputs(pcdbSnp,pcdbIndel,pcDensFilePath,unMinSupportingReads,
                                      unMaxSupportingReads,fMinReadStrength,nMinQScore,pcSampleID,
                                      pcGenomeVer,nQScoreOfs);
            continue;
        }
