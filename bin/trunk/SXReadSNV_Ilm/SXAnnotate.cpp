...
{    
    char *pcTab,*pcRec,*pChr;  size_t recstart,recstop; int i, nRecSize=0;
    ...
    while(m_pcAnnotate[index])
    {
        ...
        if (!(recstart > qstop) && !(recstop < qstart)){ 
            nRecSize = m_Records.nCurrSize+i+1;  // 1 for null

            if (m_Records.nTotalSize < nRecSize) 
            {
                do {
                     m_Records.nTotalSize += REC_SIZE;
                } while (m_Records.nTotalSize < nRecSize);
...
void SXAnnotate::GetExceptionRecord(char cChar, size_t qstart, size_t qstop)
{
    ...
    {   
            ...
            !(qstop < pExceptionIdx->pExceptionData[i].nStart))
        {
            ...
            nRecSize = m_Records.nCurrSize+nLen+1;  // 1 for null
