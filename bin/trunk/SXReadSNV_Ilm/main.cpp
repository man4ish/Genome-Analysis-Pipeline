...
static void banner(char *argv[])
{
    ...
    printf("\t-f <Input file name>\n"
           ...
           "\t    <Sample-ID> <Genome version eg. 37.1> <Output Dir> <Project SampleID>\n"
           ...
           "\t      <Max Supporting Reads> <Min Read Stregth> <Min Quality Score> <Sample-ID> <Genome version eg. 37.1>\n"
           "\t      <Quality Score Offset> <Human Ref. files path> <Output Dir> <Project SampleID>\n" 
           "\t-gLM <Binned Annotatation filename> <Binned Exception Annonatation filename> <Annotation filename>\n"
           "\t     <Min Supporting Reads to be printed> <Max Supporting Reads to be printed> <MNV Type eg. M,I,D>\n"
           "\t     <Sample-ID> <Genome version eg. 37.1> <Output Dir> <Project SampleID>\n" 
           "\t-gLMa <dbSNP database> <dbINS database> <dbDEL database> <Density Files' Path> <Min Supporting Reads>\n"
           "\t      <Max Supporting Reads> <Min Read Stregth> <Min Quality Score> <Sample-ID> <Genome version eg. 37.1>\n"
           "\t      <Quality Score Offset> <Output Dir> <Project SampleID>\n"
    ...
    for (int i = 1; i < argc; i++)
    {
		...
		banner(argv); exit(9);
            ...
            pObj->setSNVLstInputs(pcBinnedFile,pcExceptionFile,pcAnnotateFile,unMinSupportingReads,
                                  unMaxSupportingReads,cType,pcSampleID,pcGenomeVer,pcOutputDir,pcProjSampleID);
            continue;
	}
  
        if (strcmp(argv[i], "-gLM") == 0) {
           if (i >= argc - 10) {
                fprintf(stderr, "Invalid Parameters.\n");
                banner(argv); exit(9);
            }
            ...
            pObj->setMNVLstInputs(pcBinnedFile,pcExceptionFile,pcAnnotateFile,unMinSupportingReads,
                                  unMaxSupportingReads,cType,pcSampleID,pcGenomeVer,pcOutputDir,pcProjSampleID);
            continue;
        }
        ...
        if (strcmp(argv[i], "-gLMa") == 0) {
            ...
            if (strcmp(pcProjSampleID,"-c")==0){
                fprintf(stderr, "Invalid Parameters.\n");
                banner(argv); exit(9);
            }


            pObj->setAvgQSLstInputsMNV(pcdbSnp,pcdbINS,pcdbDEL,pcDensFilePath,unMinSupportingReads,
                                       unMaxSupportingReads,fMinReadStrength,nMinQScore,pcSampleID,
                                       pcGenomeVer,nQScoreOfs,pcOutputDir,pcProjSampleID);
            continue;
        }
