...
typedef struct _stMNVRec{
    unsigned int unOffset;
    char *pcRefBase;
    char *pcVarBase;
    unsigned char ucQryPos;
    unsigned char *pucQltyScore;
    unsigned short usPEnd;
    char cStrand;

    _stMNVRec()
    {
        pcRefBase=pcVarBase=NULL; pucQltyScore=NULL;
        unOffset=0;ucQryPos=0;usPEnd=0; cStrand=0;   
    }   

    ~_stMNVRec()
    {
         if (pcRefBase) free(pcRefBase);
         if (pcVarBase) free(pcVarBase);
         if (pucQltyScore) free(pucQltyScore);
    } 
}stMNVRec;


typedef struct _stMNVClusterRec{
    unsigned int unOffset;
    unsigned int unCnt;
    unsigned int unFwdCnt;
    unsigned int unRvsCnt;
...
enum eMNVType{eMNP='M',eMINS='I',eMDEL='D', eMALL='A'};
