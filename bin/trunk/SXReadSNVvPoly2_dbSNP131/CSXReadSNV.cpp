 ...
 */
 ...
 CSXReadSNV::CSXReadSNV(){
   ...
   m_pcBinnedFile=m_pcAnnotateFile=m_pcdbSnp=m_pcdbINS=m_pcdbDEL=m_pcSampleID=0;
...
{
    FILE *pfdbSnp=NULL, *pfdbINS=NULL, *pfdbDEL=NULL; char acFileName[1024];
    ...
    pfdbSnp = fopen(m_pcdbSnp,"r");
    if (!pfdbSnp) { fprintf(stderr,"Failed to open %s ...\n",m_pcdbSnp); goto Exit; }

    pfdbINS = fopen(m_pcdbINS,"r");
    if (!pfdbINS) { fprintf(stderr,"Failed to open %s ...\n",m_pcdbINS); goto Exit; }
   
    pfdbDEL = fopen(m_pcdbDEL,"r");
    if (!pfdbDEL) { fprintf(stderr,"Failed to open %s ...\n",m_pcdbINS); goto Exit; }
    ...
    for (int i=0; i<24;i++)
    {
        ...
        if (!m_apfRD[i]){fprintf(stderr,"Failed to open %s ...\n", acFile); goto Exit; }
        else fprintf(stderr,"Touch %s ...\n",acFile);
    }    
    ...
    for (short i=0;i<24;i++)
    {
        if (i < 22){ sprintf(acFileName,"%s/hs_ref_chr%d.fa",m_pcHsRefFilePath,i+1); }
        else if(i == 22) sprintf(acFileName,"%s/hs_ref_chrX.fa",m_pcHsRefFilePath);
        else sprintf(acFileName,"%s/hs_ref_chrY.fa",m_pcHsRefFilePath);

        m_apfHsRef[i] = fopen(acFileName,"r");

        if (m_apfHsRef[i]){ fprintf(stdout,"Touch %s ...\n",acFileName); }
...
Exit:
    ...
    if (pfdbSnp) fclose(pfdbSnp); if (pfdbINS) fclose(pfdbINS); if (pfdbDEL) fclose(pfdbDEL);
    ClrDBSNPMap(); m_pObjdbINS->ClrDBINDELMap(); m_pObjdbDEL->ClrDBINDELMap();
    ...
    for (unsigned long i=0; i < (merge_list_used+1); i++)
    {
        ...
        else if (m_pSNVClustered[i].cRefBase =='-' )
        {
          if (bINS1){
            ...
            bdbSNP=m_pObjdbINS->OutputINDELNovel(m_pSNVClustered[ChkPoly_INS.ulIdx].cChromosome,ChkPoly_INS.unOffset,
...
EndINS:
	    ...
	    if (bDEL1){
            ...
            bdbSNP=m_pObjdbDEL->OutputINDELNovel(m_pSNVClustered[ChkPoly_DEL.ulIdx].cChromosome,ChkPoly_DEL.unOffset,
