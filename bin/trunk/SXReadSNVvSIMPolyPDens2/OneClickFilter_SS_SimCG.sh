...
if [ $M_TYPE != "S" ]; then
   ...
   eval mv $LIST $LIST".org"
   eval mv $LIST".merge" $LIST   
fi
...
if [ "$M_TYPE" = "S" ]; then
   ...
   echo $PRG $M_FILTER_LIST $M_FILTER_LIST".Graph"
   time $PRG $M_FILTER_LIST $M_FILTER_LIST".Graph"  
#else
