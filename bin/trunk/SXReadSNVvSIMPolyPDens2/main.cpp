...
static void banner(char *argv[])
{
    ...
    printf("\t-f <Input file name>\n"
           ...
           " <Max Supporting Reads> <Min Read Stregth> <Min Quality Score> <Sample-ID> <Ambiguity filename>"
           " <Genome version eg. 37.1> <Quality Score Offset> <Human Ref. files path>\n"
...
static void parseCmdLine(CSXReadSNV *pObj, int argc, char *argv[])
{
    for (int i = 1; i < argc; i++)
    {
		...
		banner(argv); exit(9);
            ...
            const char *pcAmbiguityFileName = argv[++i];
...
///fprintf(stdout,"%s\n",pcHsRefFilePath);
            ...
            pObj->setAvgQSLstInputs(pcdbSnp,pcdbIndel,pcDensFilePath,unMinSupportingReads,
                                    ...
                                    pcGenomeVer,nQScoreOfs,pcHsRefFilePath,pcAmbiguityFileName);
            continue;
	}
